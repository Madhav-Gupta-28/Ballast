/**
 * MIRRORED from `bot/src/pricing.ts` — keep the two identical.
 *
 * The page must derive quotes by exactly the same rules the quoter uses, or it
 * would be showing a compression figure the vault never actually posts. Copied
 * rather than imported because the bot and the web app are separate packages;
 * `bot/src/pricing.test.ts` is the suite that guards it.
 */
/**
